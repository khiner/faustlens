...
TEST_CASE("iteration at zero count is two different answers") {
    // Empty parallel, sum, and product iterations yield a 0-to-0 circuit.
    ...
    // Evaluate zero-count seq bodies to determine bus width.
...
TEST_CASE("iteration unrolls in the reference's order") {
    ...
    // Preserve the unfolded iteration chain.
...
TEST_CASE("redefining a name within one layer is an error") {
    ...
    // Reject duplicate definitions within a layer, including equal definitions.
...
TEST_CASE("labels are evaluated, not copied") {
    // Match printf field-width space padding.
...
TEST_CASE("arity is checked at construction and `Error` absorbs its neighbours") {
    ...
    CHECK(!Accepts("process = _,_ :> _,_,_;"));
    CHECK(!Accepts("process = _,_ <: _,_,_;"));
